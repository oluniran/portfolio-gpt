...
const visual: Record<CaseStudy["brand"], { main: string; gallery: GalleryImage[]; heroScheme: string; detailScheme: string; galleryScheme: string }> = {
  ...
  landmark: { main: "/design/images/landmark-realty-portfolio-item-body-section-0.jpg", gallery: [{ src: "/design/images/landmark-realty-portfolio-item-body-section-1.png" }, { src: "/design/images/landmark-realty-portfolio-item-body-section-2.png" }, { src: "/design/images/landmark-realty-portfolio-item-body-section-3.jpg" }, { src: "/design/images/landmark-realty-beach-crowd.jpeg" }, { src: "/design/images/landmark-realty-beach-dusk.jpeg" }, { src: "/design/images/landmark-realty-beach-cabana-event.jpeg" }], heroScheme: "scheme-3", detailScheme: "scheme-6", galleryScheme: "scheme-3" },
};
...
const approachHeading: Record<CaseStudy["brand"], string> = {
  laserglow: "From memory-based compliance to visual intelligence.",
  alerzo: "Turning retailer data into a system.",
  landmark: "Turning footfall into a destination.",
};

const galleryHeading: Record<CaseStudy["brand"], string> = {
  laserglow: "The systems behind the story.",
  alerzo: "The growth engine behind the numbers.",
  landmark: "The place behind the numbers.",
};
...
export function CaseStudyPage({ data }: { data: CaseStudy }) {
  ...
  return <article className={brandClass[data.brand]}>
    ...
    <section className={`site-section ${media.detailScheme}`}><div className="container"><p className="eyebrow eyebrow--section">Measurable outcomes</p><div className="case-metrics">{data.metrics.map((metric) => <div className="case-metric" key={metric.label}><strong>{metric.value}</strong><span>{metric.label}</span></div>)}</div>{data.clientLogos ? <div className="logo-strip"><div className="logo-strip-track">{data.clientLogos.map((logo) => <Image src={logo.src} alt={logo.name} width={120} height={48} key={logo.name} />)}{data.clientLogos.map((logo) => <Image src={logo.src} alt="" aria-hidden width={120} height={48} key={`${logo.name}-repeat`} />)}</div></div> : null}</div></section>
    <section className="site-section scheme-2"><div className="container case-content"><div><p className="eyebrow">Approach</p><h2 className="heading">{approachHeading[data.brand]}</h2>{data.callout ? <p className="body-copy">{data.callout}</p> : null}</div><ul className="tactics">{data.tactics.map((tactic) => <li key={tactic}>{tactic}</li>)}</ul></div></section>
