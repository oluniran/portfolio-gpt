...
export function CaseStudyPage({ data }: { data: CaseStudy }) {
  ...
  return <article className={brandClass[data.brand]}>
    ...
    <section className="site-section scheme-2"><div className="container case-content"><div><p className="eyebrow eyebrow--section">Projects and Workstreams</p></div><ul className="tactics">{data.tactics.map((tactic) => <li key={tactic}>{tactic}</li>)}</ul></div></section>
    {data.experiences ? <section className="site-section scheme-2"><div className="container"><p className="eyebrow eyebrow--section">Our Experiences</p><LandmarkExperiences experiences={data.experiences} /></div></section> : null}
