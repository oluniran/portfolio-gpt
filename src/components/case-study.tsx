...
import { LandmarkExperiences } from "@/components/landmark-experiences";
import { AlerzoProjectTiles } from "@/components/alerzo-project-tiles";
...
const visual: Record<CaseStudy["brand"], { main: string; gallery: GalleryImage[]; heroScheme: string; detailScheme: string; galleryScheme: string }> = {
  ...
  alerzo: { main: "/design/images/alerzo-project-item-body-section-0.png", gallery: [{ src: "/design/images/alerzo-gallery-section-0.jpg" }, { src: "/design/images/alerzo-gallery-section-2.jpg" }, { src: "/design/images/alerzo-warehouse-delivery.jpeg" }, { src: "/design/images/alerzoshop-1.jpg" }], heroScheme: "scheme-2", detailScheme: "scheme-3", galleryScheme: "scheme-2" },
...
export function CaseStudyPage({ data }: { data: CaseStudy }) {
  ...
  return <article className={brandClass[data.brand]}>
    ...
    {!data.experiences && data.projects ? <section className="site-section scheme-2"><div className="container"><p className="eyebrow eyebrow--section">Projects and Workstreams</p><AlerzoProjectTiles projects={data.projects} /></div></section> : null}
    {!data.experiences && !data.projects && data.tactics ? <section className="site-section scheme-2"><div className="container case-content"><div><p className="eyebrow eyebrow--section">Projects and Workstreams</p></div><ul className="tactics">{data.tactics.map((tactic) => <li key={tactic}>{tactic}</li>)}</ul></div></section> : null}
