...
import { ExperiencePortfolio } from "@/components/experience-portfolio";
import { AlerzoHeroGallery } from "@/components/alerzo-hero-gallery";
...
const visual: Record<CaseStudy["brand"], { main: string; gallery: GalleryImage[]; heroScheme: string; detailScheme: string; galleryScheme: string }> = {
  ...
  alerzo: { main: "/design/images/alerzo-project-item-body-section-0.png", gallery: [{ src: "/design/images/alerzo-gallery-section-0.jpg" }, { src: "/design/images/alerzo-gallery-section-2.jpg" }, { src: "/design/images/alerzo-warehouse-delivery.jpeg" }, { src: "/design/images/alerzoshop-1.jpg" }, { src: "/design/images/alerzo-project-item-body-section-0.png" }], heroScheme: "scheme-2", detailScheme: "scheme-3", galleryScheme: "scheme-2" },
...
const galleryHeading: Partial<Record<CaseStudy["brand"], string>> = {
...
export function CaseStudyPage({ data }: { data: CaseStudy }) {
  ...
  return <article className={brandClass[data.brand]}>
    {data.heroGallery ? <section className={`site-section ${media.heroScheme}`}><div className="container alerzo-hero"><div className="case-hero-copy alerzo-hero-copy">{heroText}</div><AlerzoHeroGallery images={data.heroGallery} /></div></section>
    ...
    {data.experiences ? <section className="site-section scheme-2"><div className="container"><p className="eyebrow eyebrow--section">Projects and Workstreams</p><ExperiencePortfolio experiences={data.experiences} /></div></section> : null}
    {!data.experiences && data.tactics ? <section className="site-section scheme-2"><div className="container case-content"><div><p className="eyebrow eyebrow--section">Projects and Workstreams</p></div><ul className="tactics">{data.tactics.map((tactic) => <li key={tactic}>{tactic}</li>)}</ul></div></section> : null}
    ...
    {data.clientLogos ? <section className={`site-section scheme-2${data.brand === "alerzo" ? " case-alerzo-tight" : data.brand === "landmark" ? " case-partners-tight-top" : ""}`}><div className="container"><p className="eyebrow eyebrow--section">Partners</p><div className="logo-strip"><div className="logo-strip-track">{[0, 1, 2, 3].map((copy) => data.clientLogos!.map((logo) => <Image src={logo.src} alt={copy === 0 ? logo.name : ""} aria-hidden={copy > 0 || undefined} width={120} height={48} key={`${logo.name}-${copy}`} />))}</div></div></div></section> : null}
    <section className={`site-section ${media.galleryScheme}${data.brand === "alerzo" ? " case-alerzo-tight" : ""}`}><div className="container">{galleryHeading[data.brand] ? <><p className="eyebrow">Work in context</p><h2 className="heading" style={{ marginBottom: "3rem" }}>{galleryHeading[data.brand]}</h2></> : null}{data.brand !== "alerzo" ? <div className="gallery-scroll"><div className="gallery-scroll-track">{media.gallery.map((image) => <Image src={image.src} alt="" width={1200} height={900} sizes="(min-width: 768px) 26rem, 20rem" style={image.position ? { objectPosition: image.position } : undefined} key={image.src} />)}{media.gallery.map((image) => <Image src={image.src} alt="" aria-hidden width={1200} height={900} sizes="(min-width: 768px) 26rem, 20rem" style={image.position ? { objectPosition: image.position } : undefined} key={`${image.src}-repeat`} />)}</div></div> : null}{data.closingLine ? <p className="compound-line">{data.closingLine.pre}<strong>{data.closingLine.emphasis}</strong></p> : null}<nav className="case-nav">{data.prev ? <Link className="focus-ring" href={data.prev.href}>← {data.prev.label}</Link> : <span />}{data.next ? <Link className="focus-ring" href={data.next.href}>{data.next.label} →</Link> : null}</nav></div></section>
  </article>;
}
