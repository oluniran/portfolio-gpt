...
export function CaseStudyPage({ data }: { data: CaseStudy }) {
  ...
  return <article className={brandClass[data.brand]}>
    ...
    <section className={`site-section ${media.galleryScheme}`}><div className="container">{galleryHeading[data.brand] ? <><p className="eyebrow">Work in context</p><h2 className="heading" style={{ marginBottom: "3rem" }}>{galleryHeading[data.brand]}</h2></> : null}<div className="gallery-scroll"><div className="gallery-scroll-track">{media.gallery.map((image) => <Image src={image.src} alt="" width={1200} height={900} sizes="(min-width: 768px) 26rem, 20rem" style={image.position ? { objectPosition: image.position } : undefined} key={image.src} />)}{media.gallery.map((image) => <Image src={image.src} alt="" aria-hidden width={1200} height={900} sizes="(min-width: 768px) 26rem, 20rem" style={image.position ? { objectPosition: image.position } : undefined} key={`${image.src}-repeat`} />)}</div></div>{data.closingLine ? <p className="compound-line">{data.closingLine.pre}<strong>{data.closingLine.emphasis}</strong></p> : null}<nav className="case-nav">{data.prev ? <Link className="focus-ring" href={data.prev.href}>← {data.prev.label}</Link> : <span />}{data.next ? <Link className="focus-ring" href={data.next.href}>{data.next.label} →</Link> : null}</nav></div></section>
  </article>;
}
